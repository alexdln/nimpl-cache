import type CacheHandlerType from "@nimpl/cache-redis";

import { type CacheKeyInfo, type KeysData } from "./lib/types";
import { readStream } from "./lib/stream";

export const getKeys = async (cacheHandler: typeof CacheHandlerType): Promise<KeysData> => {
    const keys = await cacheHandler.redisLayer.getKeys();
    return keys;
};

export const getKeyDetails = async (cacheHandler: typeof CacheHandlerType, key: string): Promise<CacheKeyInfo> => {
    try {
...
export const getCacheData = (cacheHandler: typeof CacheHandlerType, segments?: string[]) => {
